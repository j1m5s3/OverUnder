...
pointers:
  ...
  - "[backend/app/indexer/listener.py : L20-42]"
...
- [STUB] `index_once` polls factory logs when connected; `run_indexer_loop` is started from `main.py` lifespan. [backend/app/indexer/listener.py : L161-210]
- [SHIPPED] AMM history: `PoolSeeded` stores the first `PricePoint` at 0.5; each `Swap` stores pool-mid `yesReserve / (yes+no)` via `pools(conditionId)` at that block, never trade-implied amounts. [backend/app/indexer/listener.py : L20-42]
