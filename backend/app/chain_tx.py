...
so a caller that sends a tx and then reads state (or treats the receipt as final)
must wait until the receipt's block is the canonical block at its height.

Reads that decide whether to send a follow-up tx from the same account should also
use block_identifier="pending": a same-sender follow-up is ordered after the tx it
depends on by nonce, so the pending view is the one that tx will execute against.
"""
